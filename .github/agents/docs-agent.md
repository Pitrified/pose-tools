...
- **Key Patterns:** Modular architecture in `src/`; configuration in `src/pose_tools/config/`; testing in `tests/`.
- **Relevant Folders:**
  - `src/pose_tools/` – Main application code
  - `src/pose_tools/config/` – Configuration management
...
- Ensure `.env` file is not present in the root, must be in `~/cred/pose-tools/.env`.
- Secrets should not be committed.

## Related

- `src/pose_tools/config/pose_tools_paths.py`
