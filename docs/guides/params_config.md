...
|---|---|---|
| **Config** | `src/pose_tools/config/` | Defines the *shape* of settings as Pydantic models |
| **Params** | `src/pose_tools/params/` | Loads *actual values* and produces a Config via `to_config()` |
...
- [`SampleConfig`](../../reference/pose_tools/config/sample_config/) - config side
- [`SampleParams`](../../reference/pose_tools/params/sample_params/) - params side
...
from pose_tools.data_models.basemodel_kwargs import BaseModelKwargs
...
1. **Single constructor argument**: `env_type: EnvType | None = None`.
   Pass it explicitly from the caller (e.g., `PoseToolsParams`).  When
...
from pose_tools.params.env_type import (
    EnvLocationType, EnvStageType, EnvType,
    UnknownEnvLocationError, UnknownEnvStageError,
)
from pose_tools.config.sample_config import NestedModel, SampleConfig
...
`PoseToolsParams` (the top-level singleton) owns all Params instances
...
```python
from pose_tools.params.env_type import EnvLocationType, EnvStageType, EnvType
